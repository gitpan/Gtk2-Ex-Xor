...
$toplevel->signal_connect
  (button_press_event =>
   sub {
     my ($widget, $event) = @_;
     print "crosshair-image.pl: start button $widget\n";
     $cross->start ($event);
   });
$toplevel->signal_connect
  (key_press_event =>
   sub {
     ...
     if ($event->keyval == Gtk2::Gdk->keyval_from_name('Escape')) {
       print "crosshair-image.pl: start key\n";
       $cross->end;

     } elsif ($event->keyval == Gtk2::Gdk->keyval_from_name('space')) {
       print "crosshair-image.pl: end key\n";
       $cross->start ($event);

     } elsif ($event->keyval == Gtk2::Gdk->keyval_from_name('r')) {
       print "crosshair-image.pl: redraw top left\n";
...
$cross->signal_connect (moved => sub {
                          print "crosshair-image.pl: moved ",
...
$cross->signal_connect
  (notify => sub {
     ...
     print "crosshair-image.pl: cross property $pname now ",
